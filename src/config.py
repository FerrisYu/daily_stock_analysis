...
class Config:
    ...
    # 大盘复盘市场区域：auto(自动识别)、cn(A股)、us(美股)、both(两者)，us 适合仅关注美股的用户
    market_review_region: str = "auto"
    ...
    def _parse_llm_channels(cls, channels_str: str) -> List[Dict[str, Any]]:
        ...
        for raw_name in channels_str.split(','):
            ...
            if not api_keys:
                ...
                _logger.debug(f"DEBUG::No multi API keys found for channel '{ch_name}', checking single key LLM_{ch_upper}_API_KEY: '{single_key}'")
                if single_key:
                    api_keys = [single_key]
